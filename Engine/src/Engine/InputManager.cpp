...
void sendEventToComponents( std::shared_ptr<Component> comp, Input::Key key )
{
    comp->OnKeyEvent( key );
}

void sendEventToGameObject( std::shared_ptr<GameObject> pGameObject, Input::Key key )
{
    pGameObject->getComponentVisitor<Input::Key>().visit( sendEventToComponents, key );
}
...
void key_callback(GLFWwindow* pWindow, int keyCode, int scancode, int actionCode, int mods)
{
    ...
    pInputManager->getKeyVisitor().visit( sendEventToGameObject, key );
}
...
InputManager::InputManager( GLFWwindow* pWindow, Engine* pEngine ) :
    m_keyVisitor(pEngine->getGameObjectVisitor<Input::Key>())
{
